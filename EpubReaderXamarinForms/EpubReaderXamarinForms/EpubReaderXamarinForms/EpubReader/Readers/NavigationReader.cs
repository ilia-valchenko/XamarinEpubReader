...
namespace EpubReaderXamarinForms.EpubReader.Readers
{
    ...
    {
        public static async Task<EpubNavigation> ReadNavigationAsync(string filepath, string contentDirectoryPath, EpubPackage package)
        {
            ...
            using (Stream containerStream = zipper.GetStream(filepath, tocFileEntryPath))
            {
                containerDocument = await XmlUtils.LoadDocumentAsync(containerStream).ConfigureAwait(false);
            }
            /* -------------------------------------- */
